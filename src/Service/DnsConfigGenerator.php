...
class DnsConfigGenerator
{
    ...
    /**
     * Returns the current Unix timestamp as the zone serial.
     ...
     * Pass $zoneName to override the zone apex name (used for alias zones that share
     * the primary domain's records but carry a different name in $ORIGIN and SOA).
     * Pass null for $view to include all records regardless of view.
     */
    public function generateZoneFile(Domain $domain, ?DnsView $view = null, ?string $zoneName = null): string
    {
        ...
        $lines   = [];
        $lines[] = '; Zone file for ' . $effectiveName . ($zoneName ? ' (alias of ' . $domain->getName() . ')' : '') . ($view ? ' (view: ' . $view->getName() . ')' : '');
        $lines[] = '; Generated by DashDDI at ' . date('Y-m-d H:i:s T');
        $lines[] = '';
        $lines[] = '$ORIGIN ' . rtrim($effectiveName, '.') . '.';
     ...
     * For secondary servers, emits flat zone stubs only — no view blocks, ACLs, or DNSSEC policies.
     */
    public function generateViewsConf(DnsServer $server): string
    {
        if ($server->isSecondary()) {
            return $this->generateSecondaryConf($server);
        }
        ...
        foreach ($server->getViews() as $view) {
            ...
            foreach ($domains as $domain) {
                ...
                foreach ($domain->getAliases() as $alias) {
                    $aliasFile = $zonePath . '/' . $view->getName() . '/' . $alias->getName() . '.zone';
                    $lines[] = '    zone "' . $alias->getName() . '" IN {';
                    if ($isSecondary) {
                        $lines[] = '        type secondary;';
                        $lines[] = '        file "' . $aliasFile . '";';
                        if ($primaryHost) {
                            $lines[] = '        primaries { ' . $primaryHost . '; };';
                        }
                    } else {
    ...
    private function generateSecondaryConf(DnsServer $server): string
    {
        $zonePath    = rtrim($server->getRemoteZonePath(), '/');
        $primaryHost = $server->getPrimaryHostname();

        $lines   = [];
        $lines[] = '// BIND zones configuration — generated by DashDDI at ' . date('Y-m-d H:i:s T');
        $lines[] = '// Include in named.conf:  include "' . $zonePath . '/dashddi.conf";';
        $lines[] = '';
