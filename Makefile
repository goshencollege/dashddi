...
test-setup: .env.test.local
	...
	docker compose -f $(COMPOSE) exec -T db mysql -u root -p$(ROOT_PASS) -e "CREATE DATABASE IF NOT EXISTS \`dashddi_test\`; GRANT ALL PRIVILEGES ON \`dashddi_test\`.* TO 'dash'@'%'; FLUSH PRIVILEGES;"
	...
	docker compose -f $(COMPOSE) exec -T app php bin/console doctrine:fixtures:load --env=test --group=test --no-interaction
